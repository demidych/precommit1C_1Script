...
Перем ВыборDiff, ДеревоCf, ДеревоМетаданных, ДеревоОбъектовКонфигурации, ДеревоОбъектовОбновления, ЗавершитьРаботу1СПослеЗакрытия, ИспользоватьКэш, КаталогДляРаспаковкиКонфигурации
, КаталогДляРаспаковкиОбновления, КаталогТретий, КоллекцияВременныхФайлов, ПроверятьАктуальнуюВерсиюПриОткрытии, РежимВыбораОбъекта
, РежимРазбора, РежимСравнения, СравнениеМетаданных, СтруктурноеСравнениеМодулей, ТретьеДерево, ТретьеДеревоОбъектов, ТрехуровневоеСравнение, ФайлКонфигурации, ФайлОбновления, ФайлТретий
, Фильтр, ФильтрТрехуровневый, МакетРасширенийТаблица;
//Перем ПараметрЗапуска;
Перем ЭтаФорма, ЭлементыФормы, РежимСверткиЭлементаУправления, РежимДиалогаВыбораФайла, ОтображениеЗакладок, НачальноеОтображениеДерева, СоответствиеИдентификаторов, СпособСравненияФайлов
, РежимДиалогаВопрос, КодВозвратаДиалога, WebЦвета, ДопустимаяДлина, ТипФайлаТабличногоДокумента;

Функция ПолучитьФорму(П1, П2 = "")
КонецФункции

Функция ПодключитьОбработчикОжидания(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция СохранитьЗначение(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция ЗавершитьРаботуСистемы(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция Предупреждение(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция ЗначениеИзСтрокиВнутр(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция Состояние(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция ЗначениеИзФайла(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция ЗначениеВФайл(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция ОбработкаПрерыванияПользователя(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция ПолучитьМакет(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция Вопрос(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции

Функция Выполнить(П1 = "", П2 = "", П3 = "", П4 = "")
КонецФункции
...
Процедура ПриОткрытии(ПараметрЗапуска) Экспорт
...
Процедура Распаковка(ФайлРаспаковки,КаталогРаспаковки)
	...
	Если КаталогРаспаковки="" тогда
		...
		Если  ИнтересующийКаталог.Существует() тогда
			...
			ГСЧ = Новый ГенераторСлучайныхЧисел(ТекущаяДата()-'19700101');
...
Процедура ОбработатьФайлПоРасширению(ТекКаталог,СтрокаРодителя,м,Режим,ч)
	...
	ОбластьИмя = СтрокаРодителя.ВидМетаданных;
	Если МакетРасширенийТаблица.Найти(ОбластьИмя, "Область") = Неопределено Тогда
		ОбластьИмя = "Остальные";
	КонецЕсли;
	МакетРасширенийТаблицаСтрока = МакетРасширенийТаблица.НайтиСтроки(Новый Структура("Область, _1", ОбластьИмя, Расширение))[0];
	МакетРасширений_Область_ЗначениеЯчейки_Текст = МакетРасширенийТаблицаСтрока._2;
	МакетРасширений_Область_ЗначениеЯчейкиСинонима_Текст = МакетРасширенийТаблицаСтрока._5;
	МакетРасширений_Область_ЗначениеЯчейкиИндексаКартинки_Текст = МакетРасширенийТаблицаСтрока._4;
	МакетРасширений_Область_ЗначениеЯчейкиОбъекта_Текст = МакетРасширенийТаблицаСтрока._3;
	НашаСтрока=СтрокаРодителя.Строки.Найти(МакетРасширений_Область_ЗначениеЯчейки_Текст,"Наименование");
	...
	Если НашаСтрока=Неопределено тогда
		...
		ПодчиненнаяСтрока.Наименование=МакетРасширений_Область_ЗначениеЯчейки_Текст;
		ПодчиненнаяСтрока.Объект=МакетРасширений_Область_ЗначениеЯчейкиОбъекта_Текст;
		...
		Иначе ПодчиненнаяСтрока.Синоним=СтрокаРодителя.Синоним+"."+МакетРасширений_Область_ЗначениеЯчейкиСинонима_Текст;
		КонецЕсли;
		//ПодчиненнаяСтрока.ИндексКартинки=МакетРасширений.Область(ЗначениеЯчейкиИндексаКартинки).Текст;
		ПодчиненнаяСтрока.ИндексКартинки=МакетРасширений_Область_ЗначениеЯчейкиИндексаКартинки_Текст;
...
Процедура ИзменитьЗаголовокDiff() Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		// туду ЭлементыФормы.НастройкаDiff.ЦветТекста=WebЦвета.Красный;
		// туду ЭлементыФормы.НастройкаDiff.Заголовок="Необходимо настроить вызов программы для трехуровневого сравнения";
	КонецПопытки;
...
Процедура РазложитьПоПапкамСогласноИерархииМетаданных(ПутьРаспаковки, ДеревоОбъектов, Знач ПутьКСохранению)
	...
	Mapping.Колонки.Добавить("Object");
	Mapping.Колонки.Добавить("File");
	Mapping.Колонки.Добавить("Begin");
	Mapping.Колонки.Добавить("End");
	...
	Для каждого ЭлементМассива Из МассивФайлов Цикл
		...
		Иначе
			...
			Если СтрокаМетаданного.Объект = "М" Тогда
				...
				Иначе
					...
					Иначе
						...
						Иначе	// ДвоичныеДанные (М1), HTMLДокумент (М3), ActiveDocument (М2) 
							ПутьНовый = НовыйПутьМакетаБезРасширения + ".txt";
						КонецЕсли;
			...
			Если СтрокаМетаданного.Объект = "Ф" И НЕ ЭлементМассива.ЭтоКаталог() И Не ПустаяСтрока(ЭлементМассива.Расширение) Тогда 
				...
				XMLТело = СтрокаВнутрВХМЛТело(_Текст);
				...
				ПолучитьСтрокиМодуляУФ(_Текст,НачальнаяСтрокаМодуляУФ,КонечнаяСтрокаМодуляУФ);
...
Процедура ПолучитьСтрокиМодуляУФ(_Текст,НачальнаяСтрока, КонечнаяСтрока)
...
Функция МакетРасширенийТаблица()
	ОбластиРасширения =
	"БизнесПроцесс;.5;Справка;С;100;Справка
	|БизнесПроцесс;.6;Модуль объекта;МОД;42;МодульОбъекта
	|БизнесПроцесс;.7;Карта маршрута;КМ;113;КартаМаршрута
	|БизнесПроцесс;.8;Модуль менеджера;МОД;42;Менеджер
	|ЖурналДокументов;.0;Справка;С;100;Справка
	|ЖурналДокументов;.1;Модуль менеджера;МОД;42;Менеджер
	|ЖурналДокументов;.2;Модуль менеджера;МОД;42;Менеджер
	|Задача;.5;Справка;С;100;Справка
	|Задача;.6;Модуль объекта;МОД;42;МодульОбъекта
	|Задача;.7;Модуль менеджера;МОД;42;Менеджер
	|Интерфейс;.0;Интерфейс;И;;Интерфейс
	|Команда;.2;Модуль команды;МОД;42;МодульКоманды
	|Макет;.0;Макет;М;53;Макет
	|МодульЗадачи;.0;Модуль приложения;МОД;42;МодульПриложения
	|МодульЗадачи;.1;Логотип;К;101;Логотип
	|МодульЗадачи;.2;Заставка;К;101;Заставка
	|МодульЗадачи;.3;Справка;С;100;Справка
	|МодульЗадачи;.4;Возможность изменения объектов, находящихся на поддержке;ПП;99;Поддержка
	|МодульЗадачи;.5;Модуль внешнего соединения;МОД;42;МодульВнешнегоСоединения
	|МодульЗадачи;.6;Модуль управляемого приложения;МОД;42;МодульУправляемогоПриложения
	|МодульЗадачи;.7;Модуль  сеанса пользователя;МОД;42;МодульСеансаПользователя
	|МодульЗадачи;.8;Рабочая область рабочего стола;РО;99;РабочаяОбластьРС
	|МодульЗадачи;.9;Объекты подсистем на поддержке;ПП;99;ПоддержкаПодсистем
	|МодульЗадачи;.a;Командный интерфейс конфигурации;КИ;0;КомандныйИнтерфейсКонфигурации
	|МодульЗадачи;.b;Интерфейс клиентского приложения;КП;0;ИнтерфейсКлиентскогоПриложения
	|МодульЗадачи;.c;Картинка основного раздела;К;101;КартинкаОсновногоРаздела
	|ОбщаяКартинка;.0;Картинка;К;101;Картинка
	|ОбщаяКоманда;.1;Справка;С;100;Справка
	|ОбщаяКоманда;.2;Модуль команды;МОД;42;МодульКоманды
	|ОбщаяФорма;.0;Форма;Ф;115;Форма
	|ОбщаяФорма;.1;Справка;С;100;Справка
	|ОбщийМакет;.0;Макет;М;53;Макет
	|Остальные;.0;Модуль объекта;МОД;42;МодульОбъекта
	|Остальные;.1;Справка;С;100;Справка
	|Остальные;.2;Модуль менеджера;МОД;42;Менеджер
	|ПакетXDTO;.0;Текст пакета;XML;;ТекстПакета
	|ПланВидовРасчета;.0;Модуль объекта;МОД;42;МодульОбъекта
	|ПланВидовРасчета;.1;Справка;С;100;Справка
	|ПланВидовРасчета;.2;Предопределенные;П;99;Предопределенные
	|ПланВидовРасчета;.3;Модуль менеджера;МОД;42;Менеджер
	|ПланВидовХарактеристик;.5;Справка;С;100;Справка
	|ПланВидовХарактеристик;.7;Предопределенные;П;99;Предопределенные
	|ПланВидовХарактеристик;.15;Модуль объекта;МОД;42;МодульОбъекта
	|ПланВидовХарактеристик;.16;Модуль менеджера;МОД;42;Менеджер
	|ПланОбмена;.0;Справка;С;100;Справка
	|ПланОбмена;.1;Состав;ССВ;99;Состав
	|ПланОбмена;.2;Модуль объекта;МОД;42;МодульОбъекта
	|ПланОбмена;.3;Модуль менеджера;МОД;42;Менеджер
	|ПланСчетов;.5;Справка;С;100;Справка
	|ПланСчетов;.9;Предопределенные;П;99;Предопределенные
	|ПланСчетов;.14;Модуль объекта;МОД;42;МодульОбъекта
	|ПланСчетов;.15;Модуль менеджера;МОД;42;Менеджер
	|Подсистема;.0;Справка;С;100;Справка
	|Подсистема;.1;Командный интерфейс;КИП;99;КомандныйИнтерфейс
	|РегистрБухгалтерии;.5;Справка;С;100;Справка
	|РегистрБухгалтерии;.6;Модуль набора записей;МОД;42;МодульНабораЗаписей
	|РегистрБухгалтерии;.7;Модуль менеджера;МОД;42;Менеджер
	|РегистрНакопления;.0;Справка;С;100;Справка
	|РегистрНакопления;.1;Модуль набора записей;МОД;42;МодульНабораЗаписей
	|РегистрНакопления;.2;Модуль менеджера;МОД;42;Менеджер
	|РегистрНакопления;.3;Агрегат;АГР;114;Агрегат
	|РегистрРасчета;.0;Справка;С;100;Справка
	|РегистрРасчета;.1;Модуль набора записей;МОД;42;МодульНабораЗаписей
	|РегистрРасчета;.2;Модуль менеджера;МОД;42;Менеджер
	|РегистрСведений;.0;Справка;С;100;Справка
	|РегистрСведений;.1;Модуль набора записей;МОД;42;МодульНабораЗаписей
	|РегистрСведений;.2;Модуль менеджера;МОД;42;Менеджер
	|РегламентноеЗадание;.0;Расписание;РС;102;Расписание
	|Роль;.0;Права;Р;99;Права
	|Справочник;.0;Модуль объекта;МОД;42;Объект
	|Справочник;.1;Справка;С;100;Справка
	|Справочник;.1c;Предопределенные;П;99;Предопределенные
	|Справочник;.3;Модуль менеджера;МОД;42;Менеджер
	|Стиль;.0;Состав стиля;ССВ;;СоставСтиля
	|Форма;.0;Форма;Ф;115;Форма
	|Форма;.1;Справка;С;100;Справка
	|ХранилищеНастроек;.8;Модуль менеджера;МОД;42;Менеджер";
	Таблица = Новый ТаблицаЗначений;
	Таблица.Колонки.Добавить("Область");
	Таблица.Колонки.Добавить("_1");
	Таблица.Колонки.Добавить("_2");
	Таблица.Колонки.Добавить("_3");
	Таблица.Колонки.Добавить("_4");
	Таблица.Колонки.Добавить("_5");
	Для Номер = 1 По СтрЧислоСтрок(ОбластиРасширения) Цикл
		Строка = СтрЗаменить(СтрПолучитьСтроку(ОбластиРасширения, Номер), ";", Символы.ПС);
		ТаблицаСтрока = Таблица.Добавить();
		Для Часть = 0 По 5 Цикл
			ТаблицаСтрока[Часть] = СтрПолучитьСтроку(Строка, Часть + 1);
		КонецЦикла;
	КонецЦикла;
	Возврат Таблица;
КонецФункции
...
Процедура КомандаВыполнить(Знач Команда, Выводить = Ложь) Экспорт
	...
	СкриптКоманда = "cmd /c " + Обрамить(Команда + " 2>" + Обрамить(Ошибки));
	...
	Если Результат Тогда		
		...
		+ "Лог " + Обрамить(Ошибки) + ":" + Символы.ПС + ПутьТекст(Ошибки, КодировкаТекста.OEM);
...
Функция ГитКоманда(ГитКаталог, Команда)
	...
	, "%ГитКаталог%", Обрамить(ГитКаталог)), "%Команда%", Команда);
КонецФункции

Функция ГитКомандаТекст(ГитКаталог, Команда) Экспорт
	Лог = ПолучитьИмяВременногоФайла();
	КомандаВыполнить(ГитКоманда(ГитКаталог, Команда + СтрЗаменить(" > %Лог%", "%Лог%", Лог)));
	Текст = СокрЛП(ПутьТекст(Лог, "UTF-8"));
	УдалитьФайлы(Лог);
	Возврат Текст;
КонецФункции

Функция СценарийПуть()
	Источник = Новый Файл(ТекущийСценарий().Источник);
	Возврат Источник.Путь;
КонецФункции

ФайлПрограммыРаспаковки = Новый Файл(СценарийПуть() + "\tools\v8unpack");

Ответ = ГитКомандаТекст(ТекущийКаталог(), "config core.quotepath");
Если Ответ <> "false" Тогда
...
МакетРасширенийТаблица = МакетРасширенийТаблица();
Журнал = ГитКомандаТекст(ТекущийКаталог(), "status --porcelain");
КаталогВходной = ТекущийКаталог() + "\";
КаталогВыходной = КаталогВходной + "src" + "\";
Для Номер = 1 По СтрЧислоСтрок(Журнал) Цикл
	...
	ДеревоМетаданных.Колонки.Добавить("Идентификатор");
	ДеревоМетаданных.Колонки.Добавить("ВидМетаданных");
	ДеревоМетаданных.Колонки.Добавить("ИндексКартинки");
	ДеревоМетаданных.Колонки.Добавить("ИндексКартинкиПодчиненных");
	ДеревоМетаданных.Колонки.Добавить("Объект");
	ДеревоМетаданных.Колонки.Добавить("ПутьТекОбъект");
	ДеревоМетаданных.Колонки.Добавить("Синоним");
	ДеревоМетаданных.Колонки.Добавить("Счетчик");
	ДеревоМетаданных.Колонки.Добавить("ПредставлениеРеквизита");

	ЖурналСтрока = СтрПолучитьСтроку(Журнал, Номер);
	Символ = Лев(ЖурналСтрока, 1);
	Если Символ = "M" ИЛИ Символ = "A" Тогда
		Имя = СокрЛП(Сред(ЖурналСтрока, 2));
		...
		КонецЕсли;
		ПутьВходной = КаталогВходной + Имя;
		Файл = Новый Файл(ПутьВходной);
		Если НРег(Файл.Расширение) = ".epf" ИЛИ НРег(Файл.Расширение) = ".erf" Тогда
			ПутьВыходной = КаталогВыходной + Имя;
			ПараметрЗапуска = СтрЗаменить(СтрЗаменить("decompile;pathtocf;%Вход%;pathout;%Выход%;"
			, "%Вход%", КаталогВходной + Имя), "%Выход%", ПутьВыходной);
			ПриОткрытии(ПараметрЗапуска);
			ГитКомандаТекст(ТекущийКаталог(), СтрЗаменить("add -f --all -f ""%ПутьВыходной%""", "%ПутьВыходной%", ПутьВыходной));
		КонецЕсли;
	КонецЕсли;
КонецЦикла;
