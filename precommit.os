#Использовать ОбщиеГит
#Использовать ОбщиеРаспаковка
...
Перем ВыборDiff, ДеревоCf, ДеревоМетаданных, ДеревоОбъектовКонфигурации, ДеревоОбъектовОбновления
, ЗавершитьРаботу1СПослеЗакрытия, ЗначениеЯчейкиОбъекта, ИспользоватьВнешнююПрограммуСравнения, ИспользоватьКэш
, КаталогДляРаспаковкиКонфигурации, КаталогДляРаспаковкиОбновления, КаталогТретий, КоллекцияВременныхФайлов
, МакетРасширенийТаблица, ПодчиненнаяСтрока, ПроверятьАктуальнуюВерсиюПриОткрытии, РежимВыбораОбъекта
, РежимРазбора, РежимСравнения, СоответствиеИдентификаторов, СравнениеМетаданных, СтруктурноеСравнениеМодулей, ТретьеДерево
, ТретьеДеревоОбъектов, ТрехуровневоеСравнение, ФайлКонфигурации, ФайлОбновления, ФайлТретий, Фильтр, ФильтрТрехуровневый;

Перем ЭтаФорма, ЭлементыФормы, РежимСверткиЭлементаУправления, РежимДиалогаВыбораФайла, ОтображениеЗакладок, НачальноеОтображениеДерева
, СпособСравненияФайлов, РежимДиалогаВопрос, КодВозвратаДиалога;

Функция ПолучитьФорму(П1, П2 = "", П3 = "")
КонецФункции
...
Процедура ПриОткрытии(ПараметрЗапуска)
...
Процедура Инициализация()
	...
	ФайлПрограммыРаспаковки = Новый Файл(ОбщиеРаспаковка.ПрограммаПуть());
...
Функция МакетРасширенийТаблица()
	ОбластиРасширения  = "Область;_1;_2;_3;_4;_5
...
Ответ = ОбщиеГит.ГитКомандаТекст(ТекущийКаталог(), "config core.quotepath");
...
Журнал = ОбщиеГит.ГитКомандаТекст(ТекущийКаталог(), "status --porcelain");
...
Для Номер = 1 По СтрЧислоСтрок(Журнал) Цикл
	...
	Если Символ = "M" ИЛИ Символ = "A" Тогда
		...
		Если НРег(Файл.Расширение) = ".epf" ИЛИ НРег(Файл.Расширение) = ".erf" Тогда
			...
			ОбщиеГит.ГитКомандаТекст(ТекущийКаталог(), СтрЗаменить("add -f --all ""%ПутьВыходной%""", "%ПутьВыходной%", ПутьВыходной));
		КонецЕсли;
	КонецЕсли;
КонецЦикла;